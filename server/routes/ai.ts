...
import { AIConsultationRequest, AIConsultationResponse } from '../../src/types.ts';
import { rateLimiter, sanitizeString } from '../middleware/security.ts';
...
aiRouter.post('/', rateLimiter(20, 60000), async (req, res) => {
  const { faceShape, hairTexture, lifestyle, desiredLength, beardPreference, notes } = req.body as AIConsultationRequest;
